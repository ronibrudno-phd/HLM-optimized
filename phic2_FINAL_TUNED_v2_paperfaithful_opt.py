...
def phic2_final_tuned(K, N, P_obs, checkpoint_dir, ETA_init=1.0e-6, ALPHA=1.0e-10, ITERATION_MAX=1000000):
    ...
    ETA_min = ETA_init * 1e-4   # was 1e-2; loosened to let optimizer continue
                                # at very small ETA where progress is microscopic
                                # but real.
    ...
    best_cost = cost
    best_K = K.copy()
    # Cache P_fit and P_dif at best_K so backups don't need to recompute K2P.
    # Mutated in-place when best_K is updated; restored on backup.
    best_P_fit = P_fit.copy()
    best_P_dif = P_dif.copy()
    best_iter = 0
    ...
    while iteration < ITERATION_MAX:
        ...
        if K_absmax > K_RUNAWAY_THRESHOLD:
            ...
            K = best_K.copy()
            P_fit[...] = best_P_fit   # restore from cache
            P_dif[...] = best_P_dif
            cost = best_cost
            ETA *= 0.8
        ...
        if _n_nonpos > 0:
            ...
            K = best_K.copy()
            P_fit[...] = best_P_fit   # restore from cache (no K2P recompute)
            P_dif[...] = best_P_dif
            cost = best_cost
            ETA *= 0.8
            eta_reduction_count += 1
        ...
        if cost < best_cost:
            ...
            best_cost = cost
            best_K = K.copy()
            best_P_fit[...] = P_fit   # cache for cheap backups
            best_P_dif[...] = P_dif
            best_iter = iteration
            consecutive_bad = 0
            consecutive_good += 1
        ...
        if consecutive_bad >= bad_threshold:
            # Halve-step retry: before full backup, try the midpoint between
            # best_K and current K. If that's better than best_cost, the
            # gradient direction was right but ETA was too aggressive --
            # salvage half the step.
            K_mid = 0.5 * (best_K + K)
            try:
                _, _n_nonpos_mid = K2P_inplace(K_mid, P_fit, identity)
                P_dif[...] = P_fit - P_obs
                mid_cost = cost_func(P_dif, N)
            except Exception:
                _n_nonpos_mid = 1
                mid_cost = float('inf')

            if (_n_nonpos_mid == 0 and not cp.isnan(mid_cost)
                    and mid_cost < best_cost):
                # Salvage succeeded. Accept the half-step.
                print(f"\n  [salvage] iter {iteration}: half-step from "
                      f"best_cost={best_cost:.6e} -> {float(mid_cost):.6e}")
                K = K_mid
                best_cost = float(mid_cost)
                best_K = K.copy()
                best_P_fit[...] = P_fit
                best_P_dif[...] = P_dif
                best_iter = iteration
                cost = best_cost
                ETA *= 0.6  # reduce ETA but less harshly than full backup
                eta_reduction_count += 1
                consecutive_bad = 0
                consecutive_good = 0
            else:
                ...
                print(f"      ETA: {ETA:.2e} -> {ETA*0.7:.2e}")
                K = best_K.copy()
                P_fit[...] = best_P_fit   # restore from cache
                P_dif[...] = best_P_dif
                cost = best_cost
                ETA *= 0.7  # was 0.8; full backup means we were too aggressive
                eta_reduction_count += 1
                consecutive_bad = 0
                consecutive_good = 0
    ...
    paras_fit = f"N={N} ETA_init={ETA_init:.2e} iterations={iteration} best_iter={best_iter}"
    
    del identity, P_dif
    # Return best_cost explicitly so the caller reports the right number,
    # not the last-trajectory-entry cost (which is often a bad step).
    return K, P_fit, c_traj, paras_fit, float(best_cost), int(best_iter)
...
if __name__ == "__main__":
    ...
    dataDir = f"{input_name}_phic2_paperfaithful_opt{OUTPUT_SUFFIX}"
    ...
    K_fit, P_fit, c_traj, paras_fit, best_cost_returned, best_iter_returned = phic2_final_tuned(
    ...
    with open(summary_file, 'w') as f:
        ...
        f.write(f"  Best iteration:          {best_iter_returned:,}\n")
        f.write(f"  Initial cost:            {c_traj_np[0,0]:.6e}\n")
        f.write(f"  Best cost:               {best_cost_returned:.6e}\n")
        f.write(f"  Last-step cost:          {c_traj_np[-1,0]:.6e}  (may be a bad step that terminated the run)\n")
        f.write(f"  Improvement:             {100*(c_traj_np[0,0]-best_cost_returned)/c_traj_np[0,0]:.2f}%  (using best, not last)\n\n")
        f.write(f"Results:\n")
    ...
    print(f"Best iteration:            {best_iter_returned:,}")
    print(f"Best cost:                 {best_cost_returned:.6e}")
    print(f"Improvement (best):        {100*(c_traj_np[0,0]-best_cost_returned)/c_traj_np[0,0]:.2f}%")
