...
# --- FIX 1: ENABLE UNIFIED MEMORY (System RAM fallback) ---
# This allows the GPU to use System RAM if VRAM fills up.
...
def K2P(K, G_buffer, identity):
    ...
    # --- FIX 2: FORCE FLOAT32 COMPUTATION ---
    P = (1.0 + 3.0 * G_buffer) ** (-1.5)
    return P.astype(cp.float32)

def Pdif2cost(P_dif):
    # Used shape[0] instead of global N to be safe
    cost = cp.sqrt(cp.sum(P_dif**2)) / P_dif.shape[0] 
    return cost

def phic2_optimized(K, P_obs, N, ETA=1.0e-4, ALPHA=1.0e-4, ITERATION_MAX=10000,
                   checkpoint_interval=1000, patience=100):
    stop_delta = ETA * ALPHA
    paras_fit = "%e\t%e\t%d\t" % (ETA, ALPHA, ITERATION_MAX)
    
    # Preallocate buffers in FLOAT32
    ...
    c_traj = cp.zeros((ITERATION_MAX+1, 2), dtype=cp.float32)
    c_traj[0, 0] = cost
    ...
    print(f"Starting optimization with N={N}, ETA={ETA}")
    ...
    while True:
        ...
        if iteration % checkpoint_interval == 0:
            ...
            # Clear memory cache to prevent fragmentation
        ...
        if converged or early_stop or max_iter_reached or diverged:
            if early_stop: K = best_K
            break
...
if __name__ == '__main__':
    ...
    # --- FIX 3: LOAD AS FLOAT32 ---
    # This reduces memory usage by 50% immediately
    try:
        P_obs_np = np.loadtxt(fhic, comments='#', dtype=np.float32)
        P_obs = cp.array(P_obs_np, dtype=cp.float32)
    except Exception as e:
        print(f"Fast load failed: {e}. Trying slower method...")
        P_obs = []
    ...
    N = len(P_obs)
    print(f"Matrix size: N={N} (Float32)")
    ...
    # Calculate correlation (safe method)
    try:
        mask = ~np.isnan(P_fit.flatten()) & ~np.isnan(P_obs.flatten())
        if np.any(mask):
            p1 = pearsonr(P_fit.flatten()[mask], P_obs.flatten()[mask])[0]
        else:
            p1 = 0.0
    except:
        p1 = 0.0
        
    ct = "#P_fit N %d min: %11.5e max: %11.5e pearson: %11.5e\n" % \
         (N, np.nanmin(P_fit), np.nanmax(P_fit), p1)
