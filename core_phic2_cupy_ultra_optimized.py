...
# ULTRA-MEMORY-OPTIMIZED version for extremely large matrices
# Uses float32 for intermediate calculations and chunked operations

if not len(sys.argv) == 2:
    print("usage:: python core_phic2_cupy_ultra_optimized.py normalized-HiC-Contact-Matrix")
    sys.exit()
fhic = str(sys.argv[1])
...
def K2P_ultra_efficient(K, N, use_float32=True):
    """
    Ultra memory-efficient K2P using mixed precision.
    Uses float32 for intermediate calculations, float64 only for final result.
    """
    dtype = cp.float32 if use_float32 else cp.float64
    
    # Compute d = sum of K rows (in original precision)
    d = cp.sum(K, axis=0)
    
    # Build L efficiently
    if use_float32 and K.dtype == cp.float64:
        # Convert to float32 for computation
        L = -K.astype(dtype)
    else:
        L = -K.copy()
    
    L[cp.diag_indices(N)] += d.astype(dtype)
    del d
    ...
    # Solve - this is the memory bottleneck
    identity = cp.eye(N-1, dtype=dtype)
    Q = solve(L_sub, identity)
    
    del identity, L_sub
    clear_memory()
    
    # M = 0.5 * (Q + Q.T) - do in-place
    Q += Q.T
    Q *= 0.5
    M = Q
    ...
    # Build G in chunks to save memory
    G = cp.zeros((N, N), dtype=dtype)
    
    # Process in chunks of 1000 rows to reduce memory
    chunk_size = 1000
    for i in range(1, N, chunk_size):
        end = min(i + chunk_size, N)
        chunk_rows = slice(i, end)
        
        # G[i:end, 1:N] = -2*M[i-1:end-1, :] + A + A.reshape(-1, 1)
        G[chunk_rows, 1:N] = -2 * M[i-1:end-1, :]
        G[chunk_rows, 1:N] += A
        G[chunk_rows, 1:N] += A.reshape(-1, 1)
    ...
    G *= 3
    G += 1
    cp.power(G, -1.5, out=G)
    
    if use_float32:
        P = G.astype(cp.float64)
        del G
    else:
        P = G
...
def Pdif2cost(P_dif):
    """Compute RMSE cost - use float32 for memory efficiency"""
    cost = cp.sqrt(cp.sum(P_dif.astype(cp.float32)**2)) / N
    return float(cost)

def phic2_ultra_optimized(K, P_obs, N, ETA=1.0e-4, ALPHA=1.0e-4, ITERATION_MAX=10000,
                         checkpoint_interval=1000, patience=100, use_float32=True):
    """
    Ultra memory-optimized PHi-C2
    """
    stop_delta = ETA * ALPHA
    paras_fit = "%e\t%e\t%d\t" % (ETA, ALPHA, ITERATION_MAX)
    
    # Everything in float32 except K and P_obs
    ...
    P_calc = K2P_ultra_efficient(K, N, use_float32=use_float32)
    P_dif = (P_calc - P_obs).astype(cp.float32)  # Use float32 for gradient
    ...
    print(f"Using {'float32' if use_float32 else 'float64'} for intermediate calculations")
    ...
    while True:
        ...
        # Compute new cost
        P_calc = K2P_ultra_efficient(K, N, use_float32=use_float32)
        P_dif = (P_calc - P_obs).astype(cp.float32)
        cost = Pdif2cost(P_dif)
        del P_calc
        ...
        # Track best solution (save to disk to free GPU memory)
        if cost < best_cost:
            best_cost = cost
            if iteration % 100 == 0:  # Only save periodically
                best_K_path = f"{dataDir}/best_K_temp.npy"
                cp.save(best_K_path, K)
                best_K_saved = True
            patience_counter = 0
        else:
            patience_counter += 1
        ...
        if converged:
            ...
            # Restore best K if saved
            if best_K_saved:
                best_K_path = f"{dataDir}/best_K_temp.npy"
                if os.path.exists(best_K_path):
                    K = cp.load(best_K_path)
                    os.remove(best_K_path)
            break
...
if __name__ == '__main__':
    ...
    print(f"Estimated peak GPU usage: ~{mem_per_matrix_gb * 3:.2f} GB")
    print(f"Read time: {time.time()-start_read:.2f}s")
    
    # Check if we should use float32
    use_float32 = (mem_per_matrix_gb > 10)  # Use float32 for large matrices
    if use_float32:
        print("\n*** Using float32 for intermediate calculations to save memory ***")
    ...
    phic2_alpha = 1.0e-10
    dataDir = fhic[:fhic.rfind('.')] + "_phic2_a%7.1e_ultra_optimized" % (phic2_alpha)
    ...
    K_fit, c_traj, paras_fit = phic2_ultra_optimized(
        K_fit,
        P_obs,
        N,
        ETA=1e-4,
    ...
    P_fit = K2P_ultra_efficient(K_fit, N, use_float32=False)  # Use full precision for final
